...
// Container
{
  ...
  container.addEventListener('copy', e => {
    alert('This site is copy protected! NO COPYING!!');
    e.preventDefault();
  });
  container.addEventListener('contextmenu', e => {
    alert('uh, no'), e.preventDefault();
  });
}
...
// Nav bar
{
  ...
  for (let i = 1; i < 7; i++) {
    ...
    nav[i - 1].addEventListener('mouseover', e => {
      let str = 'you are using ' + e.view.navigator.appVersion;
      alert(str);
    });
  }
  ...
  firstA.addEventListener('mouseenter', e => {
...
// CTA section
{
  ...
  ctaImg.addEventListener('click', () =>
    alert('what, you expect something to happen?'),
  );
}
