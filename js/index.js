...
// DOM2 PROJECT START
let firstEvent = document.querySelector(".cta h1");

firstEvent.addEventListener("dblclick", () => {
  firstEvent.innerHTML = "This changed thank god";
firstEvent.style.color = "red";
});

firstEvent.addEventListener("click", () =>{
firstEvent.style.color="blue";
})

firstEvent.addEventListener("mousedown", () => {
firstEvent.innerHTML = "Warning: Cuss is coming";
})

let superevent = document.querySelector(".text-content h4");

superevent.addEventListener("mouseover", () =>{
superevent.innerHTML = "WERE U EXPECTING ME";
})
superevent.addEventListener("select", () => {
superevent.style.color="purple";
})

let worstevent = document.querySelector(".text-content p");

worstevent.addEventListener("wheel", () => {
worstevent.innerHTML = "this is way shorter and not as annoying to read quite frankly i enjoy it more";
})

let bestevent = document.querySelector("#cta-img");

bestevent.addEventListener("contextmenu", () => {
bestevent.style.display = "none";
});


let goodevent = document.querySelector("#logo-img");

goodevent.addEventListener("mouseenter", () => {
goodevent.style.display = "none";
});

goodevent.addEventListener("auxclick", () => {
goodevent.style.display = "flex";
})

let lastevent = document.querySelector(".contact p");

lastevent.addEventListener("resize", () =>{
lastevent.style.color = "pink";
