...
document.querySelectorAll('nav a').forEach(function(link){link.addEventListener("mouseover", function(e){
    e.target.style.color = "red"; setTimeout(function(){e.target.style.color ="";}, 500);}, false)
});
...
document.querySelectorAll('img').forEach(function(img){img.addEventListener("mouseup", function(e){
  img.className = "";
})});
...
var counter = 0;
window.addEventListener('scroll', function(e){counter++;if(counter%60===0){
...
document.querySelectorAll('img').forEach(function(img){img.addEventListener("animationend", function(e){
  setTimeout(function(){alert("It's FUUUNNN!");
}, 100);
})});
